...
// Local build uses /app/openclaw.mjs; Railway installs globally at /openclaw/dist/entry.js
const OPENCLAW_ENTRY =
  process.env.OPENCLAW_ENTRY?.trim() || "/app/openclaw.mjs";
...
async function syncAllowedOrigins() {
  // Support both generic PUBLIC_DOMAIN and Railway-specific env var.
  const publicDomain =
    process.env.OPENCLAW_PUBLIC_DOMAIN?.trim() ||
...
function requireSetupAuth(req, res, next) {
  if (!SETUP_PASSWORD) {
    return res
      ...
      .type("text/plain")
      .send(
        "SETUP_PASSWORD is not set. Set it in your environment variables before using /setup.",
      );
  }
...
proxy.on("error", (err, _req, res) => {
  ...
  if (res && typeof res.headersSent !== "undefined" && !res.headersSent) {
    ...
    try {
      const html = fs.readFileSync(path.join(publicDir, "loading.html"), "utf8");
      res.end(html);
    } catch {
...
// Use OPENCLAW_PUBLIC_DOMAIN for generic deployments; fall back to Railway's env var.
const publicDomain =
  process.env.OPENCLAW_PUBLIC_DOMAIN?.trim() ||
  process.env.RAILWAY_PUBLIC_DOMAIN?.trim();
const PROXY_ORIGIN = publicDomain
  ? `https://${publicDomain}`
  : GATEWAY_TARGET;
  ...
  if (isConfigured()) {
    if (!isGatewayReady()) {
      try {
        ...
        return res
          .status(503)
          .sendFile(path.join(publicDir, "loading.html"));
      }
